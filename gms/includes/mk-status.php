...
if(isset($_GET['incid'])){
    $getMeldingen = $db->query("SELECT * FROM gms_log WHERE gelezen = '0' AND groep_id = '".$db->real_escape_string($_GET['incid'])."' AND urgent = 'nee' ORDER BY id ASC LIMIT 15");
}else{
    $getMeldingen = $db->query("SELECT * FROM gms_log WHERE gelezen = '0' AND groep_id = '".$fetchGroepInc['id']."' AND urgent = 'nee' ORDER BY id ASC LIMIT 15");
}
