...
if(isset($_GET['incid'])){
    ...
    (`eenheid` = "brw" OR
    `eenheid` = "brw motor" OR
    `eenheid` = "ovd-b") AND `status` = "1" AND groep_id = "'.$db->real_escape_string($_GET['incid']).'" ORDER BY `id` ASC');
}else{
    $getEenheid = $db->query('SELECT * FROM `gms_ingemeld` WHERE
    (`eenheid` = "brw" OR
    `eenheid` = "brw motor" OR
    `eenheid` = "ovd-b") AND `status` = "1" AND groep_id = "'.$fetchGroepInc['id'].'" ORDER BY `id` ASC');
}
...
<?php }
//error_log('eenheid-2.php: '.mysqli_error($db) . "\n", 3, "error/error.log");
?>
